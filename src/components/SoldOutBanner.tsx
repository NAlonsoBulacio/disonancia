...
export default function SoldOutBanner({ compact = false }: { compact?: boolean }) {
  if (compact) {
    return (
      <div className="mt-5 inline-block rounded-xl border border-[#8ed8e8]/40 bg-[#8ed8e8]/10 px-5 py-3 text-center lg:text-left">
        <p className="font-mono text-lg font-semibold uppercase tracking-wider text-[#8ed8e8]">
          {SOLD_OUT_TITLE}
        </p>
        <p className="mt-2 text-sm text-white/70">{SOLD_OUT_MESSAGE}</p>
      </div>
    );
  }
